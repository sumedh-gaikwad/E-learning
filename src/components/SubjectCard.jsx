            ...
            subjectList.map((subject) => {
              return (
                ...
                transition={{ type:'spring',stiffness:100,delay:subject.delay }}
                 ...
                 className="border rounded-lg border-secondary 20 p-4 flex justify-center items-center gap-4
                hover:!scale-105 hover:!shadow-xl duration-200 cursor-pointer">
                  <div style={
                    {backgroundColor:subject.color* "20"}
                  } className="w-10 h-10 rounded-m justify-center items-center">{subject.icon}</div>
                  <p>{subject.name}</p>
                </motion.div>
